...
happy_set = [i-2 for i in tappy_set]  # extract the NO. of all happy face
...
for i in order:
    if int(i.split('/')[-1]) in tappy_set:
        label.append(1)
    elif int(i.split('/')[-1]) in happy_set:
        label.append(1)
    elif int(i.split('/')[-1]) in sad_set:
        label.append(2)
    elif int(i.split('/')[-1]) in anger_set:
        label.append(3)
    elif int(i.split('/')[-1]) in disgusted_set:
        label.append(4)
    elif int(i.split('/')[-1]) in fear_set:
        label.append(5)
    elif int(i.split('/')[-1]) in sup_set:
        label.append(6)
    else:
        label.append(0)  # represent neutral face

label = pd.DataFrame(label)
label.to_csv(data_dir + '/label.csv', encoding='utf-8')
# read
